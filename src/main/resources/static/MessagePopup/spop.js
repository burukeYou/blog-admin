...
;(function() {
	...
	var SmallPop = function(template, style) {
		...
		this.defaults = {
			template  : null,  //消息提示的模板。可以是一个字符串，或这是HTML代码。
			style     : 'info',  //消息提示的主题样式，可以是info，success， warning 或 error。
			autoclose : true,		//是否自动关闭。
			position  : 'top-right', //toast消息提示的位置。可以是：top-right，top-left，top-center，bottom-left，bottom-center或bottom-right。
			icon      : true,  //是否显示图标。
			group     : false,	//是否对消息进行分组。
			onOpen    : false,	//toast消息提示打开时的回调函数。
			onClose   : false	//toast消息提示关闭时的回调函数spop.js
		};
